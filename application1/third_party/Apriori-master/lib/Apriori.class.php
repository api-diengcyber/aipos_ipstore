...
//    const SRC_ARFF = 4;         ///< Data from .arff file
    ...
    public function displayTransactions() {
        ...
        foreach ($this->trans as $tid => $t) {
            $str = "<br>".sprintf(' %5d  %s' . PHP_EOL, $tid, self::_join($t));
...
//        $rule['X'] = $lt; // searching by key is faster
    ...
    public function displayRules() {
        ...
        foreach ($this->rules as $X => &$rules) {
            ...
            foreach ($rules as $r) {
                $r['rule'] = $X." => ".$r['Y'];
				$cek	=	mysql_num_rows(mysql_query("SELECT * FROM apriori WHERE produk='".$X."' AND asosiasi='".$r['Y']."'"));
				if($cek < 1){
					mysql_query("INSERT INTO apriori(id,produk,asosiasi) VALUES(null,'".$X."','".$r['Y']."')") or die("gagal");
                }
               ...
               //$str .= "<li>".sprintf('%5d %23s %8.2f%% %27s %8.2f%%'.PHP_EOL, $i++, $r['set'], $r['supp'], $r['rule'], $r['conf'])."</li>";
			   if($i<=$limit){
				   $str .= "																	<!-- Activity item -->
							<li class='double'>
								<span class='glyphicons activity-icon cart_in'><i></i></span>
									<span class='ellipsis'>
										".$i." - ".$r['rule']." - ".$r['conf']."%
										<span class='meta'>".$r['set']." - <a>".$r['supp']."%</a></span>
									</span>
								</span>
