...
class Pembelian extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->view->render_gudang('pembelian2/pembelian_list', $data);
	}
    ...
    public function read($id_faktur = NULL)
    {
        ...
        $this->view->render_gudang('pembelian2/pembelian_read', $data);
    }
	...
	public function create()
	{
        ...
        $this->view->render_gudang('pembelian2/pembelian_form', $data);
	}
		...
		if ($row_produk) {
			...
			if ($row_temp) {
				...
				$data_insert = array(
					...
					'id_users' => $this->userdata->id_users,
					'harga_satuan' => $row_produk->harga_beli,
					'jumlah' => 1,
					'diskon' => 0,
					'expire_date' => "16-09-2099",
					'subtotal' => $row_produk->harga_beli,
				);
	...
	public function update_temp()
	{
		...
		$row_temp = $this->db->select('*')
							 ->from('pembelian_temp pt')
							 ->join('produk_retail as pr','pr.id_produk_2 = pt.id_produk')
							 ->where('pt.id',$id)
							 ->get()->row();
		if($row_temp){
			$jumlah = $this->input->post('jumlah', true);
			$subtotal = $jumlah * $row_temp->harga_beli;
			$data_update = array(
	...
	public function create_action()
	{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (count($res_temp) > 0) {
            	...
            	redirect('gudang/pembelian/create');
            } else {
	...
	public function update($id_faktur = '')
	{
		...
		if ($row_fr) {
	        ...
	        $this->view->render_gudang('pembelian2/pembelian_form_update', $data);
		} else {
	...
	public function update_temp_update()
	{
		...
		if($row_temp){
			$jumlah = $this->input->post('jumlah', true);
			$harga_satuan = $row_temp->harga_beli;
