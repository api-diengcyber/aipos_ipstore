...
class Laporan_retail extends CI_Controller {
	...
	public function stok_produk()
	{
        $data = array(
            ...
            'contents' => $this->db->select('pr.*, sat.satuan AS satuan_produk,(SELECT SUM(stok) as stok FROM stok_produk sp WHERE pr.id_produk_2=sp.id_produk AND sp.id_toko=pr.id_toko) as stok')
									->from('produk_retail pr')
									->join('users u', 'pr.id_users=u.id_users AND pr.id_toko=u.id_toko')
									->join('pembelian p', 'pr.id_produk_2=p.id_produk AND p.id_users=u.id_users AND pr.id_toko=p.id_toko', 'left')
									// ->join('stok_produk sp', 'pr.id_produk_2=sp.id_produk AND sp.id_pembelian=p.id_pembelian AND sp.id_toko=pr.id_toko', 'left')
									->join('satuan_produk sat', 'pr.satuan=sat.id_satuan AND sat.id_users=u.id_users AND sat.id_toko=pr.id_toko', 'left')
									->where('pr.id_toko', $this->userdata->id_toko)
									->where('u.id_cabang', $this->userdata->id_cabang)
									->group_by('pr.id_produk')
									->get()->result()
        );
        $this->view->render_gudang('laporan/stok_produk', $data);
	}
	...
	public function cetak_stok_produk()
	{
		...
		$data['contents'] = $this->db->select('pr.*, sp.satuan AS satuan_produk, SUM(stok.stok) AS stok')
									->from('produk_retail pr')
									->join('users u', 'pr.id_users=u.id_users AND pr.id_toko=u.id_toko')
									->join('pembelian p', 'pr.id_produk_2=p.id_produk AND p.id_users=u.id_users AND pr.id_toko=p.id_toko', 'left')
									...
									->get()->result();
		$this->load->view('gudang/laporan/cetak_stok_produk', $data, FALSE);
	}
	...
	public function stok_mati()
	{
        $data = array(
            ...
            'id_modul' => $this->userdata->id_modul,
            'nama_modul' => $this->userdata->nama_modul,
            'contents' => $this->db->select('pr.*, sp.satuan AS satuan_produk, (SELECT SUM(stok) as stok FROM stok_produk sp WHERE pr.id_produk_2=sp.id_produk AND sp.id_toko=pr.id_toko) as stok')
									->from('produk_retail pr')
									->join('users u', 'pr.id_users=u.id_users AND pr.id_toko=u.id_toko')
									->join('satuan_produk sp', 'pr.satuan=sp.id_satuan AND sp.id_users=u.id_users AND sp.id_toko=pr.id_toko')
									->join('pembelian p', 'pr.id_produk_2=p.id_produk AND p.id_users=u.id_users AND pr.id_toko=p.id_toko', 'left')
									// ->join('stok_produk stok', 'pr.id_produk_2=stok.id_produk AND stok.id_pembelian=p.id_pembelian AND stok.id_toko=pr.id_toko', 'left')
									->where('pr.id_toko', $this->userdata->id_toko)
									->where('u.id_cabang', $this->userdata->id_cabang)
        ...
        $this->view->render_gudang('laporan/stok_mati', $data);
	}
	...
	public function penjualan()
	{
		...
		// if($this->userdata->level=="1"){
			...
			if(!empty($this->input->post('kasir'))){
				...
				$data['contents'] = $this->Penjualan_retail_model->get_by_between_date_and_user($this->userdata->id_toko, $this->input->post('kasir', true), $s_awal_periode, $s_akhir_periode);
			} else {
				$data['contents'] = $this->Penjualan_retail_model->get_by_between_date($this->userdata->id_toko, $s_awal_periode, $s_akhir_periode);
			}
	        $this->view->render_gudang('laporan/penjualan_admin', $data);
		// } else{
	 ...
	 //        $this->view->render_gudang('laporan/penjualan_kasir', $data);
		// }
	}
    ...
    public function penjualan_karyawan() {
        ...
        $this->view->render_gudang('laporan/penjualan_karyawan', $data);
    }
	...
	public function detail_faktur($no_faktur)
	{
		...
		if($row){
			...
			$data = array(
				...
				'detail_order' => $this->db->select('od.*, pr.nama_produk, pr.mingros, pr.harga_1, pr.harga_2, pr.harga_3, pr.diskon AS diskon_produk, o.diskon_member, rjd.jumlah AS jumlah_retur, rjd.subtotal AS total_retur')
										...
										->join('produk_retail pr', 'od.id_produk=pr.id_produk_2 AND pr.id_users=u.id_users AND pr.id_toko=od.id_toko')
										->join('pembelian b', 'pr.id_produk_2=b.id_produk AND b.id_users=u.id_users AND b.id_toko=pr.id_toko')
										->join('retur_jual_detail rjd', 'od.id_orders=rjd.id_orders_detail AND rjd.id_users=u.id_users AND od.id_toko=rjd.id_toko', 'left')
										...
										->group_by('pr.id_produk_2')
										->get()->result(),
				'cetak' => site_url('gudang/penjualan_retail/cetak_nota_penjualan/'.$no_faktur.''),
			);
	        $this->view->render_gudang('laporan/detail_faktur_v2', $data);
		} else {
			redirect('gudang/laporan_retail/penjualan','refresh');
		}
	}
	...
	public function cetak_rekap_penjualan()
	{
		...
		if ($submit=="excel") {
			redirect("gudang/produk_retail/excel_rekap_bulan/".$bulan."/".$tahun);
		} else {
			...
			$data = array(
				...
				'data_order' => $this->db->select('o.*, u.first_name')
											...
											->where('o.id_cabang', $this->userdata->id_cabang)
			...
			$this->load->view('gudang/laporan/cetak_rekap', $data);
		}
	}
	...
	public function kasir()
	{
        ...
        $this->view->render_gudang('laporan/kasir', $data);
	}
	...
	public function pembelian()
	{
		...
		$data['awal_periode'] = $awal_periode;
		$data['akhir_periode'] = $akhir_periode;
        $this->view->render_gudang('laporan/pembelian', $data);
	}
	...
	public function buku_besar_1()
	{
        ...
        $this->view->render_gudang('laporan/buku_besar_1', $data);
	}
	...
	public function cetak_buku_besar_1()
	{
		if($this->userdata){
			...
			$this->load->view('gudang/laporan/cetak_buku_besar_1', $data);
		} else {
			$this->load->view('/');
		}
	}
	...
	public function buku_besar_2()
	{
        ...
        $this->view->render_gudang('laporan/buku_besar_2', $data);
	}
	...
	public function cetak_buku_besar_2()
	{
		if($this->userdata){
			...
			$this->load->view('gudang/laporan/cetak_buku_besar_2', $data);
		} else {
			$this->load->view('/');
		}
	}
	...
	public function neraca()
	{
        ...
        $this->view->render_gudang('laporan/neraca', $data);
	}
	...
	public function labarugi()
	{
        ...
        $this->view->render_gudang('laporan/laba_rugi', $data);
	}
	...
	public function piutang()
	{
		...
		$data['awal_periode'] = $awal_periode;
		$data['akhir_periode'] = $akhir_periode;
        $this->view->render_gudang('laporan/piutang', $data);
	}
	...
	public function hutang()
	{
        ...
        $this->view->render_gudang('laporan/hutang', $data);
	}
	...
	public function salesman()
	{
        ...
        $this->view->render_gudang('laporan/sales', $data);
	}
	...
	public function detail_sales($id_user, $tgl)
	{
		...
		$res_od_is_null = $this->db->select('od.*, m.nama, m.alamat, pr.nama_produk, SUM(rjd.jumlah) AS jumlah_retur')
									->from('orders_detail od')
									->join('users u', 'od.id_karyawan=u.id_users AND od.id_toko=u.id_toko')
									->join('produk_retail pr', 'od.id_produk=pr.id_produk_2 AND pr.id_users=u.id_users AND pr.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->join('orders o', 'od.id_orders_2=o.id_orders_2 AND o.id_users=u.id_users AND o.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->join('retur_jual_detail rjd', 'rjd.id_orders_detail=od.id_orders AND rjd.id_users=u.id_users AND rjd.id_toko=od.id_toko', 'left')
									->join('member m', 'o.pembeli=m.id_member AND m.id_users=u.id_users AND m.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->where($where_cl)
									->where('u.id_cabang', $this->userdata->id_cabang)
        ...
        $this->view->render_gudang('laporan/detail_sales', $data);
	}
	...
	public function get_detail($id_produk, $id_member, $bulan, $tahun)
	{
		$data_produk_jual = $this->db->select('SUM(od.jumlah+od.jumlah_bonus) AS terjual, SUM(od.subtotal) AS total, SUM(rjd.jumlah) AS jumlah_retur')
								...
								->join("orders_detail od", "p.id_produk_2=od.id_produk AND u.id_users=od.id_karyawan AND od.id_toko='".$this->userdata->id_toko."'")
								->join("orders o", "o.id_orders_2=od.id_orders_2 AND o.id_users=u.id_users AND o.id_toko='".$this->userdata->id_toko."'")
								->join('retur_jual_detail rjd', 'od.id_orders=rjd.id_orders_detail AND rjd.id_users=u.id_users AND od.id_toko=rjd.id_toko', 'left')
	...
	public function produk()
	{
		...
		if ($print=="true") {
			$this->load->view('gudang/laporan/ts_produk', $data, FALSE);
		} else {
	        $this->view->render_gudang('laporan/ts_produk', $data);
		}
	}
	...
	public function cetak_ts_produk()
	{
		...
		$this->load->view('gudang/laporan/ts_produk_cetak', $data, FALSE);
	}
