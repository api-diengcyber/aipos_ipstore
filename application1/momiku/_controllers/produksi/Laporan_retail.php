...
class Laporan_retail extends CI_Controller {
	...
	public function stok_produk()
	{
        $data = array(
            ...
            'contents' => $this->db->select('produk_retail.*, satuan_produk.satuan AS satuan_produk, SUM(stok_produk.stok) AS stok')
									->from('produk_retail')
									->join('stok_produk', 'produk_retail.id_produk_2=stok_produk.id_produk and stok_produk.id_toko=produk_retail.id_toko', 'left')
									->join('satuan_produk', 'produk_retail.satuan=satuan_produk.id_satuan and satuan_produk.id_toko=produk_retail.id_toko', 'left')
									->where('produk_retail.id_toko', $this->userdata->id_toko)
									->group_by('stok_produk.id_produk')
									->get()->result()
        );
        $this->view->render_produksi('laporan/stok_produk', $data);
	}
	...
	public function cetak_stok_produk()
	{
		...
		$data['contents'] = $this->db->select('produk_retail.*, satuan_produk.satuan AS satuan_produk, SUM(stok_produk.stok) AS stok')
									->from('produk_retail')
									->join('stok_produk', 'produk_retail.id_produk_2=stok_produk.id_produk and stok_produk.id_toko=produk_retail.id_toko', 'left')
									->join('satuan_produk', 'produk_retail.satuan=satuan_produk.id_satuan and satuan_produk.id_toko=produk_retail.id_toko', 'left')
									->where('produk_retail.id_toko', $this->userdata->id_toko)
									->group_by('stok_produk.id_produk')
									->get()->result();
		$this->load->view('produksi/laporan/cetak_stok_produk', $data, FALSE);
	}
	...
	public function stok_mati()
	{
        ...
        $this->view->render_produksi('laporan/stok_mati', $data);
	}
	...
	public function penjualan()
	{
		...
		// if($this->userdata->level=="1"){
			$data['data_kasir'] = $this->db->where('id_toko', $this->userdata->id_toko)
									->where('level', '2')
									->get('users')->result();
			if(!empty($this->input->post('kasir'))){
				$data['kasir'] = $this->input->post('kasir');
				$data['contents'] = $this->Penjualan_retail_model->get_by_between_date_and_user($this->userdata->id_toko, $this->input->post('kasir'), $s_awal_periode, $s_akhir_periode);
			} else {
				$data['contents'] = $this->Penjualan_retail_model->get_by_between_date($this->userdata->id_toko, $s_awal_periode, $s_akhir_periode);
			}
	        $this->view->render_produksi('laporan/penjualan_admin', $data);
		// } else{
	 ...
	 //        $this->view->render_produksi('laporan/penjualan_kasir', $data);
		// }
	}
    ...
    public function penjualan_karyawan() {
        ...
        $this->view->render_produksi('laporan/penjualan_karyawan', $data);
    }
	...
	public function detail_faktur($no_faktur)
	{
		...
		if($row){
			...
			$data = array(
				...
				'detail_order' => $this->db->select('orders_detail.*, produk_retail.nama_produk, produk_retail.mingros, produk_retail.harga_1, produk_retail.harga_2, produk_retail.harga_3, produk_retail.diskon AS diskon_produk, orders.diskon_member, rjd.jumlah AS jumlah_retur, rjd.subtotal AS total_retur')
										->from('orders_detail')
										->join('orders', 'orders_detail.id_orders_2=orders.id_orders_2 AND orders.id_toko=orders_detail.id_toko')
										->join('produk_retail', 'orders_detail.id_produk=produk_retail.id_produk_2 AND produk_retail.id_toko=orders_detail.id_toko')
										->join('pembelian AS b', 'produk_retail.id_produk_2=b.id_produk AND b.id_toko=produk_retail.id_toko')
										->join('retur_jual_detail rjd', 'orders_detail.id_orders=rjd.id_orders_detail AND orders_detail.id_toko=rjd.id_toko', 'left')
										...
										->group_by('produk_retail.id_produk_2')
										->get()->result(),
				'cetak' => site_url('penjualan_retail/cetak_nota_penjualan/'.$no_faktur.''),
			);
	        $this->view->render_produksi('laporan/detail_faktur_v2', $data);
		} else {
			redirect('produksi/laporan_retail/penjualan','refresh');
		}
	}
	...
	public function cetak_rekap_penjualan()
	{
		...
		if ($submit=="excel") {
			redirect("produksi/produk_retail/excel_rekap_bulan/".$bulan."/".$tahun);
		} else {
			...
			$data = array(
				...
				'tahun' => $tahun,
				'data_order' => $this->db->select('orders.*, users.first_name')
											->from('orders')
											->join('users', 'orders.id_users=users.id_users AND users.id_toko=orders.id_toko')
											->where('orders.id_toko', $this->userdata->id_toko)
											->where('SUBSTRING(orders.tgl_order,7,4)='.$tahun)
											->where('SUBSTRING(orders.tgl_order,4,2)='.$bulan)
											->order_by('orders.id_orders_2', 'asc')
											->get()->result(),
			);
			$this->load->view('produksi/laporan/cetak_rekap', $data);
		}
	}
	...
	public function kasir()
	{
		$data = array(
            ...
            'id_modul' => $this->userdata->id_modul,
            'nama_modul' => $this->userdata->nama_modul,
			'data_order_per_kasir' => $this->db->select('COUNT(orders.id_orders_2) AS jml_order, users.first_name, users.last_name')
											->from('orders')
											->join('users', 'orders.id_users=users.id AND users.id_toko=orders.id_toko')
											->group_by('orders.id_users')
											->where('orders.id_toko', $this->userdata->id_toko)
											->get()->result(),
        ...
        $this->view->render_produksi('laporan/kasir', $data);
	}
	...
	public function pembelian()
	{
		...
		$data['awal_periode'] = $awal_periode;
		$data['akhir_periode'] = $akhir_periode;
        $this->view->render_produksi('laporan/pembelian', $data);
	}
	...
	public function buku_besar_1()
	{
		...
		$where_akun = 'akun.id > 0';
		if ($this->userdata->level!="5") {
			$where_akun = 'akun.kode!="2.03"';
		} else {
			$where_akun = 'akun.kode!="2.04"';
		}
		$data = array(
            'id_toko' => $this->userdata->id_toko,
            'nama_toko' => $this->userdata->nama_toko,
			...
			'data_akun' => $this->db->select('akun.id, akun.kode, akun.akun')
									->from('akun')
									->join('jurnal', 'akun.id=jurnal.id_akun AND jurnal.id_toko=akun.id_toko')
									->where('jurnal.id_toko', $this->userdata->id_toko)
									->where($where_akun)
									->group_by('jurnal.id_akun')
									->order_by('akun.kode')
									->get(),
			'data_saldo_awal' => $this->db->select("id_akun, tgl, SUM(debet)-SUM(kredit) AS saldo")
										  ->from("jurnal")
        ...
        $this->view->render_produksi('laporan/buku_besar_1', $data);
	}
		...
		if($this->userdata){
			...
			$data = array(
				...
				'data_akun' => $this->db->select('akun.id, akun.kode, akun.akun')
										->from('akun')
										->join('jurnal', 'akun.id=jurnal.id_akun AND jurnal.id_toko=akun.id_toko')
										->where('jurnal.id_akun', $this->userdata->id_toko)
										->where($where_akun)
										->group_by('jurnal.id_akun')
										->order_by('akun.kode')
										->get(),
			...
			$this->load->view('produksi/laporan/cetak_buku_besar_1', $data);
		} else {
			$this->load->view('/');
		}
	}
	...
	public function buku_besar_2()
	{
		...
		$data = array(
			...
			'data_akun' => $this->db->select('akun.id, akun.kode, akun.akun')
									...
									->order_by('akun.kode')
									->get(),
			'akun_saldo_awal' => $this->db->select("jurnal.id_akun, jurnal.tgl, SUM(jurnal.debet)-SUM(jurnal.kredit) AS saldo, akun.kode")
										  ->from("jurnal")
										  ->join("akun", "jurnal.id_akun = akun.id")
										  ->where("DATE(CONCAT(SUBSTRING(jurnal.tgl,7,4),'-',SUBSTRING(jurnal.tgl,4,2),'-',SUBSTRING(jurnal.tgl,1,2))) < '".$awal_periode."' ")
										  ->group_by("jurnal.id_akun")
										  ->order_by('jurnal.kode')
										  ->get()->result(),
			'data_saldo' => $this->db->select("id_akun, tgl, SUM(debet)-SUM(kredit) AS saldo")
									  ->from("jurnal")
        ...
        $this->view->render_produksi('laporan/buku_besar_2', $data);
	}
	...
	public function cetak_buku_besar_2()
	{
		if($this->userdata){
			...
			$this->load->view('produksi/laporan/cetak_buku_besar_2', $data);
		} else {
			$this->load->view('/');
		}
	}
	...
	public function neraca()
	{
		...
		$data = array(
			...
			'data_saldo' => $this->db->select("id_akun, tgl, SUM(debet)-SUM(kredit) AS saldo")
										  ->from("jurnal")
										  ->where('id_toko', $this->userdata->id_toko)
										  ->where("DATE(CONCAT(SUBSTRING(tgl,7,4),'-',SUBSTRING(tgl,4,2),'-',SUBSTRING(tgl,1,2))) BETWEEN '".$s_per_01."' AND '".$s_per."' ")
										  ->group_by("id_akun")
										  ->get(),
			'data_activa' => $this->db->where($where_akun)->where("SUBSTRING(kode,1,1)", "1")->order_by('kode', 'asc')->get("akun"),
			'data_pasiva' => $this->db->where($where_akun)->where("SUBSTRING(kode,1,1)", "2")->where("kode !=", "2.05")->order_by('kode', 'asc')->get("akun"),
			'data_pendapatan' => $this->db->where($where_akun)->where("SUBSTRING(kode,1,1)", "3")->order_by('kode', 'asc')->get("akun"),
			'data_biaya' => $this->db->where($where_akun)->where("SUBSTRING(kode,1,1)", "4")->order_by('kode', 'asc')->get("akun"),
		);
        $this->view->render_produksi('laporan/neraca', $data);
	}
	...
	public function labarugi()
	{
        ...
        $this->view->render_produksi('laporan/laba_rugi', $data);
	}
	...
	public function piutang()
	{
		...
		$data['awal_periode'] = $awal_periode;
		$data['akhir_periode'] = $akhir_periode;
        $this->view->render_produksi('laporan/piutang', $data);
	}
	...
	public function hutang()
	{
        ...
        $this->view->render_produksi('laporan/hutang', $data);
	}
	...
	public function salesman()
	{
        ...
        $this->view->render_produksi('laporan/sales', $data);
	}
	...
	public function detail_sales($id_user, $tgl)
	{
		...
		$res_od_is_null = $this->db->select('od.*, m.nama, m.alamat, pr.nama_produk, SUM(rjd.jumlah) AS jumlah_retur')
									->from('orders_detail od')
									->join('produk_retail pr', 'od.id_produk=pr.id_produk_2 AND pr.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->join('orders o', 'od.id_orders_2=o.id_orders_2 AND o.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->join('retur_jual_detail rjd', 'rjd.id_orders_detail=od.id_orders AND rjd.id_toko=od.id_toko', 'left')
									->join('member m', 'o.pembeli=m.id_member AND m.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->where($where_cl)
									// ->group_by('od.id_orders_2')
									->order_by('od.id_orders_2', 'asc')
									->get()->result();
		$res_od_not_null = $this->db->select('od.*, m.nama, m.alamat, pr.nama_produk, SUM(rjd.jumlah) AS jumlah_retur')
									->from('orders_detail od')
									->join('produk_retail pr', 'od.id_produk=pr.id_produk_2 AND pr.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->join('orders o', 'od.id_orders_2=o.id_orders_2 AND o.id_toko="'.$this->userdata->id_toko.'"', 'left')
									->join('retur_jual_detail rjd', 'rjd.id_orders_detail=od.id_orders AND rjd.id_toko=od.id_toko', 'left')
        ...
        $this->view->render_produksi('laporan/detail_sales', $data);
	}
	...
	public function produk()
	{
		...
		if ($print=="true") {
			$this->load->view('produksi/laporan/ts_produk', $data, FALSE);
		} else {
	        $this->view->render_produksi('laporan/ts_produk', $data);
		}
	}
	...
	public function cetak_ts_produk()
	{
		...
		$this->load->view('produksi/laporan/ts_produk_cetak', $data, FALSE);
	}
