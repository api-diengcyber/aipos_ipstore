...
class Akun_retail extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['akun_level_5'] = $this->db->select('*')
                        ...
                        ->get()->result();
        $data['action'] = site_url('produksi/akun_retail/create_action');
        $this->view->render_produksi('akun/akun_list', $data);
    }
    ...
    public function json() {
        ...
        echo $this->Akun_retail_model->json_produksi($this->userdata->id_toko);
    }
    ...
    public function read($id) 
    {
        ...
        if ($row) {
            ...
            $this->view->render_produksi('akun/akun_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('produksi/akun_retail'));
        }
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                ...
                'nama_modul' => $this->userdata->nama_modul,
                'button' => 'Simpan',
                'action' => site_url('produksi/akun_retail/update_action'),
        		'id' => set_value('id', $row->id),
        		'id_toko' => set_value('id_toko', $row->id_toko),
        		'kode' => set_value('kode', $row->kode),
        		'akun' => set_value('akun', $row->akun),
    	    );
            $this->view->render_produksi('akun/akun_form', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('produksi/akun_retail'));
        }
