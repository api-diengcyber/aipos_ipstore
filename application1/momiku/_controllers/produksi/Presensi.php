...
class Presensi extends CI_Controller {
	...
	public function index()
	{
        ...
        $data = array(
            ...
            'data_karyawan' => $this->db->select('u.*, jk.jam_masuk, jk.jam_pulang, jk.tidak_masuk, jk.status, jk.keterangan')
            							->from('users u')
            							->join('jam_kerja jk', 'u.id_users=jk.id_pegawai AND jk.id_toko=u.id_toko AND jk.tgl="'.$tgl.'"', 'left')
	...
	private function pulang($id_users, $tgl)
	{
		$row = $this->db->select('jk.*')
						->from('jam_kerja jk')
						->where('jk.id_toko', $this->userdata->id_toko)
