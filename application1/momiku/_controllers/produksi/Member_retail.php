...
class Member_retail extends CI_Controller
{
    ...
    public function json() {
        ...
        echo $this->Member_retail_model->json_produksi($this->userdata->id_toko);
    }
    ...
    public function index()
    {
        ...
        $this->view->render_produksi('member/member_list', $data);
    }
    ...
    public function json_input_sales_temp()
    {
        ...
        if ($row) {
            $row_cek = $this->db->select('*')
                                ...
                                ->where('id_users', $this->userdata->id_user)
                                ->where('id_produk', $row->id_produk_2)
                                ->where('acc_sales', 0)
                                ->get()->row();
            $row_produk = $this->db->select('produk_retail.*, satuan_produk.satuan AS satuan_produk, SUM(stok_produk.stok) AS stok')
                                    ->from('produk_retail')
                                    ...
                                    ->where('produk_retail.id_produk_2', $id_produk_2)
    ...
    public function json_sales_temp()
    {
        ...
        $res_sales_temp = $this->db->select('ost.*, p.nama_produk, SUM(stok.stok) AS stok, s.satuan AS satuan_produk')
                                    ...
                                    ->join('produk_retail p', 'ost.id_produk=p.id_produk_2 AND p.id_toko='.$this->userdata->id_toko, 'left')
                                    ->join('kategori_produk k', 'p.id_kategori=k.id_kategori_2 AND k.id_toko='.$this->userdata->id_toko, 'left')
                                    ->join('stok_produk stok', 'p.id_produk_2=stok.id_produk AND stok.id_toko='.$this->userdata->id_toko, 'left')
                                    ->join('pembelian b', 'p.id_produk_2=b.id_produk AND stok.id_pembelian=b.id_pembelian AND b.id_toko='.$this->userdata->id_toko, 'left')
                                    ->join('satuan_produk s', 'p.satuan=s.id_satuan AND s.id_toko='.$this->userdata->id_toko, 'left')
                                    ->where('ost.id_toko', $this->userdata->id_toko)
                                    ->where('ost.id_member', $id_member)
                                    ->where('ost.id_users', $this->userdata->id_user)
    ...
    {
        $data_produk = $this->db->select('produk_retail.*, satuan_produk.satuan AS satuan_produk, SUM(stok_produk.stok) AS stok')
                                ->from('produk_retail')
                                ->join('stok_produk', 'produk_retail.id_produk_2=stok_produk.id_produk and stok_produk.id_toko=produk_retail.id_toko', 'left')
                                ->join('satuan_produk', 'produk_retail.satuan=satuan_produk.id_satuan and satuan_produk.id_toko=produk_retail.id_toko', 'left')
                                ->where('produk_retail.id_toko', $this->userdata->id_toko)
        ...
        $this->view->render_produksi('member/order_sales', $data);
    }
    ...
    public function order_sales_delete($id_temp)
    {
        ...
        $this->db->where('id_users', $this->userdata->id_user);
    ...
    public function order_sales_action($id_member)
    {
        ...
        redirect('produksi/member_retail/order_sales_acc/'.$id_member);
    }
    ...
    public function order_sales_acc($id_member)
    {
        $res_sales_temp = $this->db->select('ost.*, p.nama_produk, SUM(stok.stok) AS stok')
                                    ...
                                    ->join('produk_retail p', 'ost.id_produk=p.id_produk_2 AND p.id_toko='.$this->userdata->id_toko, 'left')
                                    ->join('kategori_produk k', 'p.id_kategori=k.id_kategori_2 AND k.id_toko='.$this->userdata->id_toko, 'left')
                                    ->join('pembelian b', 'p.id_produk_2=b.id_produk AND b.id_toko='.$this->userdata->id_toko, 'left')
        ...
        $this->view->render_produksi('member/order_sales_acc', $data);
    }
    ...
    public function read($id) 
    {
        ...
        if ($row) {
            ...
            $this->db->update('orders_sales_temp', $data_update);
            $row_sales = $this->db->where('id_toko', $this->userdata->id_toko)->where('id_users', $row->id_sales)->where('level', '5')->get('users')->row();
            $nama_sales = " - ";
            if ($row_sales) {
            ...
            $data = array(
                ...
                'data_order_sales' => $this->db->select('ost.*, p.nama_produk')
                                                ->from('orders_sales_temp ost')
                                                ->join('produk_retail p', 'ost.id_produk=p.id_produk_2 AND p.id_toko=ost.id_toko')
                                                ->where('ost.id_toko', $this->userdata->id_toko)
            ...
            $this->view->render_produksi('member/member_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('produksi/member'));
        }
    }
    ...
    public function proses_orders()
    {
        ...
        redirect('produksi/penjualan_retail/create?id_member='.$id_member);
    }
    ...
    public function create() 
    {
        ...
        $data = array(
            ...
            'nama_modul' => $this->userdata->nama_modul,
            'button' => 'Tambah',
            'action' => site_url('produksi/member_retail/create_action'),
            'id' => set_value('id'),
            'id_toko' => $this->userdata->id_toko,
        ...
        $this->view->render_produksi('member/member_form', $data);
    }
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect(site_url('produksi/member_retail'));
        }
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                ...
                'nama_modul' => $this->userdata->nama_modul,
                'button' => 'Simpan',
                'action' => site_url('produksi/member_retail/update_action'),
    ...
    public function word()
    {
        ...
        $this->load->view('produksi/member/member_doc',$data);
    }
    ...
    public function kartu_pelanggan()
    {
        ...
        $this->view->render_produksi('member/member_card', $data);
    }
    ...
    public function kartu_pelanggan_action()
    {
        ...
        $this->load->view('produksi/member/cetak_member_card', $data, FALSE);
    }
    ...
    public function semua_order()
    {
        ...
        $data_sales_temp = $this->db->select('ost.id, ost.id_orders_temp, ost.tgl, SUM(ost.jumlah) AS jml_qty, m.nama, m.alamat')
                                    ...
                                    ->join('member m', 'ost.id_member=m.id_member AND m.id_toko="'.$this->userdata->id_toko.'"')
                                    ->where('ost.id_users', $this->userdata->id_user)
                                    ->where('ost.id_toko', $this->userdata->id_toko)
                                    ->where('ost.acc_sales', "1")
        ...
        $this->view->render_produksi('member/semua_order', $data);
    }
    ...
    public function semua_order_detail($id)
    {
        $row_sales_temp = $this->db->select('ost.id, ost.id_orders_temp, ost.tgl, ost.id_member, SUM(ost.jumlah) AS jml_qty, m.nama, m.alamat')
                                    ...
                                    ->get()->row();
        if (!$row_sales_temp) {
            redirect('produksi/member_retail/semua_order', 'refresh');
        }
        $jenis = 0;
        ...
        $data_sales_temp = $this->db->select('ost.*, m.nama, pr.nama_produk, s.satuan AS satuan_produk')
                                    ->from('orders_sales_temp ost')
                                    ->join('member m', 'ost.id_member=m.id_member AND m.id_toko="'.$this->userdata->id_toko.'"')
                                    ->join('produk_retail pr', 'ost.id_produk=pr.id_produk_2 AND pr.id_toko="'.$this->userdata->id_toko.'"')
                                    ->join('satuan_produk s', 'pr.satuan=s.id_satuan AND s.id_toko="'.$this->userdata->id_toko.'"', 'left')
                                    ->where('ost.id_users', $this->userdata->id_user)
                                    ->where('ost.id_toko', $this->userdata->id_toko)
                                    ->where('ost.acc_sales', "1")
        ...
        $this->view->render_produksi('member/semua_order_detail', $data);
    }
