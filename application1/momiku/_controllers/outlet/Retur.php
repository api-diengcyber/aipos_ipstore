...
class Retur extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['data_retur'] = $this->db->select('r.*, f.no_faktur, s.nama_supplier')
                                       ->from('retur r')
                                       ->join('users u', 'r.id_users=u.id_users AND r.id_toko=u.id_toko')
        ...
        $this->view->render_outlet('retur/retur_list', $data);
    }
    ...
    public function read($id)
    {
        ...
        if ($data_retur) {
            ...
            $this->view->render_outlet('retur/retur_read', $data);
        } else {
            $this->session->set_flashdata('message', 'No Record Found');
            redirect('outlet/retur');
        }
    }
	...
	public function create()
	{
        $data = array(
            ...
            'id_modul' => $this->userdata->id_modul,
            'nama_modul' => $this->userdata->nama_modul,
            'action' => site_url('outlet/retur/create_action'),
            'id' => set_value('id'),
        ...
        $this->view->render_outlet('retur/retur_form', $data);
	}
	...
	public function update($id)
	{
        ...
        if ($row) {
            $data = array(
                ...
                'id_modul' => $this->userdata->id_modul,
                'nama_modul' => $this->userdata->nama_modul,
                'action' => site_url('outlet/retur/update_action'),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $row = $this->db->select('r.*')
                          ...
                          ->where('r.id_toko', $this->userdata->id_toko)
                          ->where('u.id_cabang', $this->userdata->id_cabang)
                          ->where('r.id', $this->input->post('id', TRUE))
                          ->get()->row();
	        if ($row) {
	        	$data = array(
	        		'tgl' => $this->input->post('tgl', true),
                ...
                $row_sp_pembelian = $this->db->select('sp.*')
                                          ->from('pembelian b')
                                          ->join('users u', 'b.id_users=u.id_users AND b.id_toko=u.id_toko')
