...
class Orders_temp_retail_model extends CI_Model {
    ...
    {
        $this->db->select('ot.*, p.id_produk_2 AS produk_id, p.id_produk_2, p.barcode, p.nama_produk, p.harga_1, p.harga_2, p.harga_3, p.harga_4, p.harga_5, p.harga_6, p.diskon AS diskon_produk, p.mingros, p.dibeli');
        $this->db->from('orders_temp ot');
        $this->db->join('users u', 'ot.id_users=u.id_users AND ot.id_toko=u.id_toko');
        $this->db->join('produk_retail p', 'ot.id_produk=p.id_produk_2 AND p.id_users=u.id_users AND p.id_toko=ot.id_toko');
