...
class Penjualan_retail_model extends CI_Model
{
    ...
    function get_by_between_date($id_toko, $tgl1='', $tgl2='', $id_member = 0)
    {
        ...
        return $this->db->get()->result();
        */
        $this->db->select('o.*, od.*, u.first_name,u.last_name, (o.nominal-o.ppn_nominal) AS nominal, SUM(rjd.jumlah) AS jumlah_retur');
        $this->db->from('orders o');
        ...
        if ($this->cek_retur_jual($id_toko)) {
            ...
            $this->db->join('retur_jual r', 'r.no_faktur=o.no_faktur AND r.id_users=u.id_users AND r.id_toko=o.id_toko');
        }
        $this->db->where('o.id_toko', $this->userdata->id_toko);
