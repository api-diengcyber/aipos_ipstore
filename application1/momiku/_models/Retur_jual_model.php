...
class Retur_jual_model extends CI_Model {
    ...
    function json($id_toko,$level = "")
    {
        ...
        $this->datatables->add_column('action', anchor(site_url('admin/retur_jual/read/$1'),'<button class="btn btn-xs btn-success"><i class="ace-icon fa fa-check bigger-120"></i></button>')."&nbsp;&nbsp;&nbsp;&nbsp;".anchor(site_url('admin/retur_jual/delete/$1'),'<button class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash-o bigger-120"></i></button>','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id_retur');
        }
        $this->datatables->group_by('r.id_retur');
        return $this->datatables->generate();
    }

    function json_produksi($id_toko,$level = "")
    {
        $this->datatables->select('r.id_retur, r.id_toko, r.tgl, r.no_retur, r.no_faktur, r.total');
        $this->datatables->from('retur_jual r');
        $this->datatables->join('users u', 'r.id_users=u.id_users AND r.id_toko=u.id_toko');
