...
class Produksi_model extends CI_Model {
    ...
    function get_produk_search($term, $tgl)
    {
        ...
        $this->db->join('supplier', 'p.id_supplier=supplier.id_supplier AND supplier.id_users=u.id_users AND supplier.id_toko=p.id_toko', 'left');
        $this->db->join('faktur_retail', 'p.id_faktur=faktur_retail.id_faktur AND faktur_retail.id_users=u.id_users AND faktur_retail.id_toko=p.id_toko');
    ...
    function get_bahan_search($term, $tgl)
    {
        ...
        $this->db->from('pembelian p');
        $this->db->join('users u', 'p.id_users=u.id_users AND p.id_toko=u.id_toko');
        $this->db->join('produk_retail pr', 'p.id_produk=pr.id_produk_2 AND p.id_users=u.id_users AND pr.id_toko=p.id_toko');
        $this->db->join('supplier', 'p.id_supplier=supplier.id_supplier AND supplier.id_users=u.id_users AND supplier.id_toko=p.id_toko', 'left');
        $this->db->join('faktur_retail', 'p.id_faktur=faktur_retail.id_faktur AND faktur_retail.id_users=u.id_users AND faktur_retail.id_toko=p.id_toko');
        $this->db->where('p.id_toko', $this->userdata->id_toko);
        $this->db->where('u.id_cabang', $this->userdata->id_cabang);
