          ...
          <div class="breadcrumbs ace-save-state breadcrumbs-fixed" id="breadcrumbs">
            <ul class="breadcrumb">
              <li>
                ...
                <a href="<?php echo base_url() ?>admin/penyesuaian_stok">Home</a>
              </li>
              <li class="">Manajemen Produk</li>
              <li class="active">Penyesuaian Stok Edit</li>
            </ul><!-- /.breadcrumb -->
                ...
                <div class="row" style="margin-bottom: 10px">
                  ...
                  <div class="col-md-3">
                    <select multiple="" class="chosen-select form-control" data-placeholder="Pilih Kolom">
                      ...
                      <option value="5">Stok SO</option>
                      <option value="6">Penyesuaian</option>
                    </select>
                  </div>
                  <div class="col-md-3">
                  ...
                  <div class="col-md-3">
                    <input type="text" class="form-control" value="<?php echo $nama_cabang ?>" readonly>
                  </div>
                </div>
...
<?php 
  ...
  function negativeValueRenderer(instance, td, row, col, prop, value, cellProperties) {
    ...
    if (col==5||col==6||col==7||col==8||col==9||col==10||col==11) {
  ...
  var hotSettings = {
    ...
    columns: [
      ...
      },
      {
        data: 'stok_so',
        type: 'numeric',
        <?php if (!empty($lihat)) { ?> readOnly: true, <?php } ?>
      },
      {
        data: 'penyesuaian',
        type: 'numeric',
    ...
    afterChange: function (change, source) {
      ...
      var sourceRow = hot.getDataAtRow(change[0][0]);
      ...
      change[0].push(sourceRow[5]);
      var penyesuaian = this.getDataAtCell(rowid,7)*1;
      var new_penyesuaian = (this.getDataAtCell(rowid,6)*1) - (this.getDataAtCell(rowid,5)*1);
      // alert(JSON.stringify(new_penyesuaian));
      
      if (penyesuaian != new_penyesuaian) {
        this.setDataAtCell(rowid,7,new_penyesuaian);
  ...
  function loadData(data) {
    $.ajax({
      url: '<?php echo base_url() ?>admin/produk_retail/json_data/<?php echo $id_stok_info ?>/<?php echo $cabang ?>',
      type: 'get',
  ...
  function saveData(data, result) {
    $.ajax({
      url: '<?php echo base_url() ?>admin/penyesuaian_stok/simpan_produk',
      type: 'post',
      data: 'data='+JSON.stringify(data)+'&id_stok_info=<?php echo $id_stok_info ?>&cabang=<?php echo $cabang ?>',
      success: function(response){
        result(response);
      }
    });
