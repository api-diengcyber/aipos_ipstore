...
class Hutang_retail_model extends CI_Model
{
    ...
    function json() {
        ...
        if($this->userdata->level == 1){
            $this->datatables->add_column('action', anchor(site_url('admin/hutang_retail/read/$1'),'<button type="button" class="btn btn-success btn-xs">Lihat</button>')." ".anchor(site_url('admin/jurnal_retail/bayar_hutang_create/$1'),'<button type="button" class="btn btn-primary btn-xs">Bayar</button>')." ".anchor(site_url('admin/hutang_retail/delete/$1'),'<button type="button" class="btn btn-danger btn-xs">Hapus</button>','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id');
