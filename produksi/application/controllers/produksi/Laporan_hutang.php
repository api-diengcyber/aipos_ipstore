...
class Laporan_hutang extends CI_Controller {
	...
	public function migrasi()
	{
		...
		$res = $this->db->select('h.id AS id_hutang, h.tgl, SUM(h.kurang) AS total_hutang, s.id_supplier, s.nama_supplier, a.id AS id_akun, a.akun, fr.id_faktur, fr.no_faktur')
						...
						->join('supplier s', 'h.id_supplier=s.id_supplier AND s.id_users=u.id_users AND h.id_toko=s.id_toko')
	...
	public function rekap()
	{
		...
		if ($print!="true") {
	        $this->view->render_produksi('laporan_baru/hutang2', $data);
		} else {
			$this->load->view('produksi/laporan_baru/hutang2', $data);
		}
	}
	...
	public function rinci()
	{
		...
		$data['akhir_periode'] = $akhir_periode;
		if ($print!="true") {
	        $this->view->render_produksi('laporan_baru/hutang_rinci', $data);
		} else {
			$this->load->view('produksi/laporan_baru/hutang_rinci', $data);
		}
	}
