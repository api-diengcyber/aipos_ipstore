...
class Laporan_penjualan extends CI_Controller {
	...
	public function rinci_per_area()
	{
		...
		$data['data_kota'] = $this->db->select('*')
									  ->from('kota')
									  ->get()->result();
		$data['data_sales_from_orders'] = $this->db->select('o.*, u.email, u2.first_name, u2.last_name')
													->from('orders o')
													->join('users u', 'o.id_users=u.id_users AND o.id_toko=u.id_toko')
													->join('users u2', 'o.id_sales=u2.id_users AND o.id_toko=u2.id_toko', 'left')
													// ->join('toko t', 'o.id_toko=t.id')
													->join('member m', 'o.pembeli=m.id_member AND m.id_users=u.id_users AND o.id_toko=m.id_toko')
	...
	public function rekap_per_area()
	{
		...
		$data['data_sales_from_orders'] = $this->db->select('o.*, u.email, u2.first_name, u2.last_name')
													...
													// ->join('toko t', 'o.id_toko=t.id')
													->join('member m', 'o.pembeli=m.id_member AND u.id_users=m.id_users AND o.id_toko=m.id_toko')
