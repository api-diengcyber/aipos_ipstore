...
class Home extends CI_Controller {
	...
	public function index()
	{
        ...
        $data['produk_terlaris_hari_ini'] = $this->db->select('pr.*, SUM(od.jumlah) AS jumlah')
                                            ...
                                            ->join('orders o', 'od.id_orders_2=o.id_orders_2 AND o.id_users=u.id_users AND od.id_toko=o.id_toko')
        ...
        $data['data_stok_bahan'] = $this->db->select('pr.*, SUM(sp.stok) AS stok')
                                            ...
                                            ->join('stok_produk sp', 'pr.id_produk_2=sp.id_produk AND sp.id_pembelian=p.id_pembelian AND pr.id_toko=sp.id_toko')
                                            ->where('pr.id_toko', $this->userdata->id_toko)
                                            ->where('u.id_cabang', $this->userdata->id_cabang)
                                            ->where('pr.jenis', 1)
        ...
        $data['data_stok_produksi'] = $this->db->select('pr.*, SUM(sp.stok) AS stok')
                                            ...
                                            ->limit(6)
                                            ->get()->result();
        $data['data_stok_inkubasi'] = $this->db->select('ib.id, ib.id_toko, ib.id_users, ib.id_inkubasi, ib.id_produk, ib.expire_date, ib.keranjang, ib.cup, ib.rusak, ib.total, ib.dipakai, pr.nama_produk, SUM(ib.keranjang) AS keranjang, SUM(ib.cup) AS cup, SUM(ib.rusak) AS rusak, SUM(ib.total) AS total, SUM(ib.dipakai) AS dipakai')
                                              ->from('inkubasi_barang ib')
                                              ->join('users u', 'ib.id_users=u.id_users AND ib.id_toko=u.id_toko')
                                              ->join('produk_retail pr', 'ib.id_produk=pr.id_produk_2 AND pr.id_users=u.id_users AND ib.id_toko=pr.id_toko')
        ...
        $data['total_hutang'] = $this->db->select('SUM(h.kurang) AS total')
                                        ...
                                        ->get()->row();
		$this->view->render_produksi('home', $data);
  }
