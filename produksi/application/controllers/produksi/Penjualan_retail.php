...
class Penjualan_retail extends CI_Controller
{
    ...
    public function index()
    {
      ...
      $this->view->render_produksi('penjualan/orders_list', $data);
    }
    
    public function json() {
      header('Content-Type: application/json');
      echo $this->Penjualan_retail_model->json_produksi($this->userdata->id_toko);
    }
    ...
    public function read($id)
    {
        ...
        if ($row) {
            ...
            $this->view->render_produksi('penjualan/orders_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('produksi/penjualan_retail'));
        }
    }
    ...
    public function create() 
    {
        ...
        $data = array(
          ...
          'id_modul' => $this->userdata->id_modul,
          'button' => 'Selesai',
          'action' => site_url('produksi/penjualan_retail/create_action'),
          'id_toko' => $this->userdata->id_toko,
        ...
        $this->view->render_produksi('penjualan/form_penjualan', $data);
    }
    ...
    public function create_action() 
    {
        ...
        $row_member = $this->db->select('m.*, u2.id_users AS id_users_outlet')
                               ->from('member m')
                               ->join('users u', 'm.id_users=u.id_users AND m.id_toko=u.id_toko')
                               ->join('users u2', 'm.id_cabang=u.id_cabang AND m.id_toko=u.id_toko', 'left')
        ...
        $pkp = false;
        if ($id_pembeli*1!=0) {
          $row_member = $this->db->select('m.*, u2.id_users AS id_users_outlet, u2.id_cabang AS id_cabang_outlet')
                                 ->from('member m')
                                 ->join('users u', 'm.id_users=u.id_users AND m.id_toko=u.id_toko')
                                 ->join('users u2', 'm.id_cabang=u2.id_cabang AND m.id_toko=u2.id_toko', 'left')
                                 ->where('m.id_toko', $this->userdata->id_toko)
                                 ->where('u.id_cabang', $this->userdata->id_cabang)
                                 ->where('m.id_member', $id_pembeli)
                                 ->get()->row();
          ...
          $row_pro = $this->db->select('pr.*')
                              ->from('produk_retail pr')
                              ->join('users u', 'pr.id_users=u.id_users AND pr.id_toko=u.id_toko')
                              ->where('pr.id_toko', $this->userdata->id_toko)
                              // ->where('u.id_cabang', $this->userdata->id_cabang)
                              ->where('pr.barcode', $key->barcode)
                              ->where('u.id_users', $row_member->id_users_outlet)
                              ->get()->row();

          if ($row_member->id_users_outlet*1 > 0 && $row_pro) {
            ...
            $row_faktur = $this->db->select('fr.*')
                                   ...
                                   ->where('u.id_cabang', $row_member->id_cabang_outlet)
                                   ->where('fr.no_faktur', 'FAKTUR-DARI-PRODUKSI')
            ...
            if($row_faktur){
              ...
              $this->db->where('id', $row_faktur->id);
              $this->db->update('faktur_retail', $data_faktur);
            } else {
              // insert //
              $row_last_faktur = $this->db->select('fr.id_faktur')
                                          ->from('faktur_retail fr')
                                          ->where('fr.id_toko', $this->userdata->id_toko)
                                          // ->where('id_supplier', 0)
                                          ->order_by('fr.id_faktur', 'desc')
              ...
              $data_faktur = array(
                'id_faktur' => $id_faktur,
                'id_toko' => $this->userdata->id_toko,
                'id_users' => $row_member->id_users_outlet,
                'tgl' => date('d-m-Y'),
                'no_faktur' => 'FAKTUR-DARI-PRODUKSI',
              );
            ...
            /* Pembelian */
            $row_pembelian = $this->db->where('id_toko', $this->userdata->id_toko)
                                      ->where('id_faktur', $id_faktur)
                                      ->where('id_produk', $row_pro->id_produk_2)
                                      ->get('pembelian')->row();
            if ($row_pembelian) {
              $id_beli = $row_pembelian->id_pembelian;
              $hs = $harga_barang>0?$harga_barang/$key->jumlah:0;
              $data_beli = array(
                'tgl_masuk' => date('d-m-Y'),
                'harga_satuan' => $hs,
                'jumlah' => $row_pembelian->jumlah+($key->jumlah+$key->jumlah_bonus),
                'total_bayar' => $hs*($key->jumlah),
              );
              ...
              $data_beli = array(
                ...
                'id_users' => $row_member->id_users_outlet,
                'id_faktur' => $id_faktur,
                'id_produk' => $row_pro->id_produk_2,
                'tgl_masuk' => date('d-m-Y'),
                'tgl_expire' => date('d-m-2099'),
                'pembayaran' => '1',
                'harga_satuan' => $hs,
                'jumlah' => $key->jumlah+$key->jumlah_bonus,
                'total_bayar' => $hs*($key->jumlah),
              );
            ...
            $row_stok = $this->db->select('sp.*')
                                 ...
                                 ->where('sp.id_pembelian', $id_beli)
                                 ->where('sp.id_produk', $row_pro->id_produk_2)
                                 ->get()->row();
            if ($row_stok) {
              $data_stok = array(
                'stok' => $row_stok->stok+($key->jumlah+$key->jumlah_bonus)
              );
              ...
              $data_stok = array(
                ...
                'id_produk' => $row_pro->id_produk_2,
                'id_toko' => $this->userdata->id_toko,
                'stok' => $key->jumlah+$key->jumlah_bonus,
              );
    ...
    public function detail_faktur($faktur="")
    {
        ...
        $this->view->render_produksi('penjualan/detail_faktur', $data);
    }
    ...
    private function _cetak_nota_wprint($format_nota, $data)
    {
        if ($format_nota == 'praktis2') {
            $this->load->view('produksi/penjualan/cetak_nota_penjualan_praktis3', $data);
            //$this->load->view('produksi/penjualan/cetak_nota_penjualan_praktis2', $data);
        } else if ($format_nota == 'praktis') {
            $this->load->view('produksi/penjualan/cetak_nota_penjualan_praktis', $data);
        } else if ($format_nota == 'besar') {
            $this->load->view('produksi/penjualan/cetak_nota_penjualan_besar', $data);
        } else {
            $this->load->view('produksi/penjualan/cetak_nota_penjualan', $data);
        }
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
              ...
              'id_modul' => $this->userdata->id_modul,
              'button' => 'Simpan',
              'action' => site_url('produksi/penjualan_retail/update_action'),
            ...
            $this->view->render_produksi('penjualan/orders_form_update', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('produksi/penjualan_retail'));
        }
    ...
    public function word()
    {
        ...
        $this->load->view('produksi/penjualan_retail/orders_doc',$data);
    }
    ...
    public function json_produk()
    {
        ...
        $res = $this->Produk_retail_model->get_by_search($this->userdata->id_toko, $term, $tgl, 'bahan');
        $data = array();
        $i = 0;
        foreach ($res as $key) {
    ...
    public function json_produk_all()
    {
        header('Content-Type: application/json');
        $term = $this->input->post('term');
        $tgl = $this->input->post('tgl');
    ...
    public function orders_sales_group()
    {
        ...
        $this->view->render_produksi('penjualan/orders_sales_group', $data);
    }
    ...
    public function orders_sales($id = '')
    {
        ...
        if (!empty($id)) {
          ...
          if ($row_orders_sales) {
            ...
            redirect('produksi/penjualan_retail/orders_sales_group', 'refresh');
          }
        } else {
          $data_orders_sales = $this->db->select('os.*, p.nama_produk, m.nama, u.first_name, u.last_name')
                                        ->from('orders_sales_temp os')
        ...
        $this->view->render_produksi('penjualan/orders_sales', $data);
    }
    ...
    public function orders_sales_action()
    {
        ...
        if (!empty($btn) && $btn=="1") {         
            ...
            if (count($orders_sales) > 0) {
                ...
                //$this->client_send_ost();
                $this->session->set_flashdata('message', 'Delete Record Success');
                redirect(site_url('produksi/penjualan_retail/orders_sales'));
            } else {
                $this->session->set_flashdata('message', 'Record Not Found');
                redirect(site_url('produksi/penjualan_retail/orders_sales'));
            }
            ...
            if (count($orders_sales) > 0) {
                ...
                redirect(site_url('produksi/penjualan_retail/create?id_member='.$id_member));
            } else {
                $this->session->set_flashdata('message', 'Record Not Found');
                redirect(site_url('produksi/penjualan_retail/orders_sales'));
            }
