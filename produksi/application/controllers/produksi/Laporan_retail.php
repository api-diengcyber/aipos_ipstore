...
class Laporan_retail extends CI_Controller {
	...
	public function penjualan()
	{
		...
		if(!empty($this->input->post('awal_periode', true))){
		...
		if(!empty($this->input->post('akhir_periode', true))){
		...
		if($row){
			...
			$data = array(
				...
				'detail_order' => $this->db->select('od.*, pr.nama_produk, pr.mingros, pr.harga_1, pr.harga_2, pr.harga_3, pr.diskon AS diskon_produk, o.diskon_member, rjd.jumlah AS jumlah_retur, rjd.subtotal AS total_retur')
										...
										->group_by('pr.id_produk_2')
										->get()->result(),
				'cetak' => site_url('produksi/penjualan_retail/cetak_nota_penjualan/'.$no_faktur.''),
			);
	...
	private function x_week_range($date) {
	    ...
	    return array(date('d-m-Y', $start), date('d-m-Y', strtotime('next saturday', $start)));
	}
