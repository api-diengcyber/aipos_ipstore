...
var pil_harga = ["-- Pilih Harga --", "H. produksi", "H. Res. Kecil", "H. Res. Besar", "H. Agen Kecil", "H. Agen Besar", "H. Online"];
...
$(document).ready(function(){
  ...
  function statusBarcode() {
    ...
    if (status_barcode=="1") { // BARCODE
      ...
      source_nama_barang = function(request, response){
        $.ajax({
          url: '<?php echo base_url() ?>produksi/penjualan_retail/json_produk_all',
          type: 'post',
  ...
  $(".form_barang").submit(function(e){
    ...
    function insertTemp() {
      ...
      $.ajax({
        url: '<?php echo base_url() ?>produksi/penjualan_retail/insert_barang_temp_by_id/<?php echo date("Y-m-d") ?>',
        type: 'post',
  ...
  function update(data) {
    $.ajax({
      url: '<?php echo base_url() ?>produksi/penjualan_retail/update_temp',
      type: 'post',
      data: data,
  ...
  function loadTemp() {
    ...
    $.ajax({
      url: '<?php echo base_url() ?>produksi/penjualan_retail/load_temp',
      type: 'post',
      ...
      success: function(data) {
        ...
        var temp = data.data;
        $(".total").html(tandaPemisahTitik(Math.round(total)));
        ...
        $(".btn-delete").on('click', function(){
          ...
          var val = $(this).val();
          $.ajax({
            url: '<?php echo base_url() ?>produksi/penjualan_retail/delete_orders_temp/'+data_id,
            type: 'post',
            success: function(){
              loadTemp();
            },
  ...
  $(".btn-selesai").on('click', function(){
    if (ibarang > 0) {
      ...
      $.ajax({
        url: '<?php echo base_url() ?>produksi/penjualan_retail/create_action',
        type: 'post',
        processData: false,
        contentType: false,
        data: fd,
        success: function(data) {
          ...
          if (typeof(data.no_faktur)!=='undefined') {
            var myWindow = window.open("<?php echo base_url() ?>produksi/penjualan_retail/cetak_nota_penjualan/"+data.no_faktur, "MsgWindow", "width=300, height=400");
          }
          showPopup("Tersimpan");
          refresh();
        },
