...
class Produksi_model extends CI_Model {
    ...
    function get_produk_search($term, $tgl)
    {
        ...
        if (!empty($this->userdata->id_cabang)) {
            ...
            $this->db->join('users u', 'p.id_users=u.id_users AND p.id_toko=u.id_toko', 'left');
            $this->db->join('produk_retail pr', 'p.id_produk=pr.id_produk_2 AND pr.id_toko=p.id_toko');
            $this->db->join('supplier s', 'p.id_supplier=s.id_supplier AND s.id_toko=p.id_toko', 'left');
            $this->db->join('faktur_retail fr', 'p.id_faktur=fr.id_faktur AND fr.id_toko=p.id_toko');
        }
