                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        foreach ($data as $item) : 
                          ...
                          $item->closing = 0;
                          foreach ($data_produk as $key) :
                            $jml_produk[$key->kode_produk] += $item_arr[$key->kode_produk];
                            $item->closing += $item_arr[$key->kode_produk];
                          endforeach;
                          // $item->closing = $item->herbaskin+$item->gracilli; // replace
                          if (!empty($item->leads)) {
                              // $presentase = round((($item->gracilli + $item->herbaskin) / $item->leads)*100,2);
                              $presentase = round(($item->closing / $item->leads)*100,2);
                          }
                          $jml_leads += $item->leads;
                          $jml_totalan += $item->totalan;
                          // $jml_herbaskin += $item->herbaskin;
                          // $jml_gracilli += $item->gracilli;
                          $jml_closing += $item->closing;
                        ...
                        <tr>
                            ...
                            <?php foreach ($data_produk as $key) : ?>
                            <td class="text-center"><?php echo $item_arr[$key->kode_produk];?></td>
                            <?php endforeach ?>
                            <td class="text-center"><?php echo $item->closing;?></td>
