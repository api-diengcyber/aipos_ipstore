                ...
                <div class="col-xs-12">
                  ...
                  if($jenis != 1){
                    ...
                    for ($i=1; $i <= 12; $i++) {
                      $saldo_pendapatan = 0;
                      foreach ($data_pendapatan->result() as $key_pendapatan):
                        $row = $that->get_saldo_berjalan_bulanan($i,$tahun, $key_pendapatan->id);
                        if ($row) {
                          $saldo_pendapatan += $row->saldo*-1;
                        }
                      endforeach;
                      $saldo_beban = 0;
                      foreach ($data_biaya->result() as $key_biaya):
                        $row = $that->get_saldo_berjalan_bulanan($i,$tahun, $key_biaya->id);
                        if ($row) {
                          $saldo_beban += $row->saldo;
                        }
                      endforeach;
                      $saldo_labarugi = 0;
                      $row = $that->get_saldo_berjalan_bulanan($i,$tahun, 14);
                      if ($row) {
                        $saldo_labarugi += $row->saldo;
                      }
