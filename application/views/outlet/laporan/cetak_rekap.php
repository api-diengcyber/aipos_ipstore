	      ...
	      <table class="table" border="1">
	        ...
	        <tbody>
		        	<?php $i = 1;foreach ($data_order as $do): 
		        		if(!empty($do->pembeli)){
		        			$nama = $do->nama_pembeli.' - '.$do->alamat_pembeli;
		        		}else{
		        			$nama = $do->bukan_member;
		        		}
		        	?>
	        		<?php 
	        			$this->db->select('orders_detail.*, produk_retail.nama_produk');
						$this->db->from('orders_detail');
						$this->db->join('produk_retail', 'produk_retail.id_produk_2=orders_detail.id_produk');
						$this->db->join('pembelian', 'pembelian.id_produk=orders_detail.id_produk', 'left');
						$this->db->where('orders_detail.id_orders_2', $do->id_orders_2);
						$this->db->where('produk_retail.id_toko = '.$toko->id);
						if ($jenis == "2") {
							$this->db->where('produk_retail.id_kategori', 7);
						} else if ($jenis == "1") {
							$this->db->where('produk_retail.id_kategori !=', 7);
						}
						$this->db->group_by('orders_detail.id_orders');
						$data_order_detail = $this->db->get()->result();
