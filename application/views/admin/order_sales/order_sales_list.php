              ...
              <div class="col-xs-12">
                ...
                <div class="alert alert-info">
                  ...
                  <div class="row" style="margin-bottom: 10px">
                      ...
                      <div class="col-md-2">
                        <h6>Media</h6>
                        <select name="id_media" id="id_media" class="form-control">
                            <option value="">--</option>
                            <option value="4" <?php echo $id_media=="4"?"selected":"" ?>>COD</option>
                            <option value="1" <?php echo $id_media=="1"?"selected":"" ?>>Whatsapp</option>
                            <option value="3" <?php echo $id_media=="3"?"selected":"" ?>>Tokopedia</option>
                            <option value="2" <?php echo $id_media=="2"?"selected":"" ?>>Shopee</option>
                        </select>
                      </div>
                      <div class="col-md-2">
                        <h6>Dari Tanggal</h6>
                ...
                <?php if ($show_warning) { ?>
                <div class="alert alert-warning alert-yellow">
                  Terdapat data duplikat, cek di  <a href="<?php echo base_url() ?>admin/validasi">Validasi data</a> kemudian lakukan penyesuaian jurnal bila perlu di menu <a href="<?php echo base_url()?>admin/penyesuaian_jurnal">Penyesuaian Jurnal </a>
                      ...
                      <tr style="<?php echo $tr_style ?>">
                        ...
                        <td>
                          ...
                          <?php 
                            ...
                            if ($order_sales->id_status == 2) {
                              echo '<br><a onclick="return confirm(`Are you sure ?`)" href="'.base_url().'admin/order_sales/delete_verifikasi_pembayaran/'.$order_sales->id_order.'" class="btn btn-danger">Hapus Verf. Pembayaran</a><br>';
                              echo '<br><a onclick="return confirm(`Apakah akan mengubah status menjadi dikirim tanpa mengurangi stok ?`)" href="'.base_url().'admin/order_sales/verifikasi_status/'.$order_sales->id_order.'" class="btn btn-warning">Ubah Dikirim</a>';
                            }

                            if ($order_sales->id_status == 3) {
                              echo '<br><button type="button" class="btn btn-danger btn-delete-mdl" data-id="'.$order_sales->id_order.'">Hapus Data</button>';
                            }
                            
                            if($order_sales->id_status == 1){
                                foreach($exp as $item){
                                    $btn = "btn-primary";
                                    if($order_sales->id_expedisi === $item->id){
                                        $btn = "btn-success";
                                    }
                                    echo "<a href='".base_url('admin/order_sales/update_expedisi/'.$order_sales->id_order.'/'.$item->id.'/'.$page)."' class='btn btn-xs ".$btn."' style='margin-right:3px;margin-bottom:3px'>".$item->nama_expedisi.'</a>';
                                }
                            } ?>
                            <br><br>
                          <?php if($order_sales->id_status == 1) {?>
                          <a onclick="return confirm('Are you sure?')" href="<?php echo base_url('admin/order_sales/delete/'.$order_sales->id_order.'/'.$page);?>" class="btn btn btn-danger btn-sm">HAPUS</a>
                          <!-- <a href="<?php echo base_url('admin/order_sales/update/'.$order_sales->id_order.'/'.$page);?>" class="btn btn btn-warning btn-sm">EDIT</a> -->
                          <a href="<?php echo base_url('admin/penjualan_retail/create/'.$order_sales->id_order.'/'.$page);?>" class="btn btn btn-warning btn-sm">EDIT</a>
                          <br><br>
                          <a href="<?php echo base_url('admin/order_sales/verifikasi_pembayaran/'.$order_sales->id_order.'/'.$page);?>" class="btn btn btn-primary btn-block btn-sm btn-verifikasi-bayar">VERIFIKASI PEMBAYARAN</a>
                          
                          <a href="<?php echo base_url('admin/order_sales/saldo_terbuka/'.$order_sales->id_order.'/'.$page);?>" class="btn btn btn-primary btn-block btn-sm btn-verifikasi-bayar">SALDO TERBUKA</a>
                          <?php } ?>
