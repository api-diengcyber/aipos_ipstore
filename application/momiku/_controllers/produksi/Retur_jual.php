...
class Retur_jual extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->view->render_produksi('retur_jual/retur_jual_list', $data);
	}
	...
	public function read($id)
	{
		...
		if ($row_retur) {
            ...
            $this->view->render_produksi('retur_jual/retur_jual_read', $data);
		} else {
            $this->session->set_flashdata('message', 'Record not found');
            redirect(site_url('produksi/retur_jual'));
		}
	}
	...
	public function json()
	{
		...
		echo $this->Retur_jual_model->json_produksi($this->userdata->id_toko);
	}
...
/*
	public function create()
	{
		...
		$data = array(
			'action' => site_url().'produksi/retur_jual/create_action',
			'no_retur' => '',
            'id_modul' => $this->userdata->id_modul,
            'button' => 'Selesai',
            'id_toko' => $this->userdata->id_toko,
    ...
    public function create()
    {
        ...
        $this->view->render_produksi('retur_jual/retur_jual_form', $data);
    }
    ...
    public function create_manual($value='')
    {
        $data = array(
            ...
            'id_modul' => $this->userdata->id_modul,
            'nama_modul' => $this->userdata->nama_modul,
            'action' => site_url().'produksi/retur_jual/create_action_manual',
            'no_retur' => '',
            'id_modul' => $this->userdata->id_modul,
            'button' => 'Selesai',
            'id_toko' => $this->userdata->id_toko,
        ...
        $this->view->render_produksi('retur_jual/retur_jual_form_manual', $data);
    }
        ...
        if ($row_produk) {
            ...
            if ($stok_ada > 0) {
                ...
                $row = $this->db->select('rjt.*')
                                ->from('retur_jual_temp rjt')
                                ->where('rjt.id_toko', $this->userdata->id_toko)
                                ->where('rjt.id_users', $this->userdata->id_user)
                                ->where('rjt.id_produk', $row_produk->id_produk_2)
                                ->get()->row();
                if ($row) {
                    $data_update = array(
