...
class Laporan_persediaan extends CI_Controller {
	...
	public function index()
	{
		...
		$data['method'] = $this;
		if ($print!="true") {
	        $this->view->render_produksi('laporan_baru/persediaan', $data);
		} else {
			$this->load->view('produksi/laporan_baru/persediaan', $data);
		}
	}
	...
	public function get_detail($id_produk, $add_where)
	{
		...
		$row_r = $this->db->select('SUM(r.jumlah) AS qty_retur_beli, SUM(p.total_bayar) AS rupiah_retur_beli')
		                ->from('retur r')
		                ->join('pembelian p', 'r.id_faktur=p.id_faktur AND r.id_produk=p.id_produk AND r.id_toko=p.id_toko')
		...
		$row_rjd = $this->db->select('SUM(rjd.jumlah) AS qty_retur_jual, SUM(rjd.subtotal) AS rupiah_retur_jual')
		                ...
		                ->join('retur_jual rj', 'rjd.id_retur=rj.id_retur AND rjd.id_toko=rj.id_toko')
