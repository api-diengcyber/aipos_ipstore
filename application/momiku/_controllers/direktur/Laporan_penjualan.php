...
class Laporan_penjualan extends CI_Controller {
	...
	public function rinci_per_area()
	{
		...
		$data['data_kota'] = $this->db->select('*')
									  ->from('kota')
									  ->get()->result();
		$data['data_sales_from_orders'] = $this->db->select('o.*, u2.email, u2.first_name, u2.last_name')
													->from('orders o')
													->join('users u', 'o.id_users=u.id_users AND o.id_toko=u.id_toko')
													->join('users u2', 'o.id_sales=u.id_users AND o.id_toko=u.id_toko')
													...
													->get()->result();
		if ($print!="true") {
	        $this->view->render_direktur('laporan_baru/penjualan_rinci_per_area', $data);
		} else {
			$this->load->view('direktur/laporan_baru/penjualan_rinci_per_area', $data);
		}
	}
	...
	public function rekap_per_area()
	{
		...
		$data['data_sales_from_orders'] = $this->db->select('o.*, u2.email, u2.first_name, u2.last_name')
													...
													->get()->result();
		if ($print=="true") {
			$this->load->view('direktur/laporan_baru/penjualan_rekap_per_area', $data, FALSE);
		} else {
	        $this->view->render_direktur('laporan_baru/penjualan_rekap_per_area', $data);
		}
	}
	...
	public function rekap()
	{
		...
		$data['data_kota_from_orders'] = $this->db->select('k.*')
													...
													->where('o.id_users', $this->userdata->id_users)
													->where('u.id_cabang', $this->userdata->id_cabang)
												    ->where('DATE(CONCAT(SUBSTRING(o.tgl_order,7,4),"-",SUBSTRING(o.tgl_order,4,2),"-",SUBSTRING(o.tgl_order,1,2))) BETWEEN "'.$awal_periode.'" AND "'.$akhir_periode.'"')
													->group_by('k.id')
													->get()->result();
		if ($print=="true") {
			$this->load->view('direktur/laporan_baru/penjualan_rekap_area', $data, FALSE);
		} else {
	        $this->view->render_direktur('laporan_baru/penjualan_rekap_area', $data);
		}
	}
