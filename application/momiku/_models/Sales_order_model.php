...
class Sales_order_model extends CI_Model {
	...
	function grafik_order_hari_ini()
	{
		...
		foreach ($res_tanggal as $tanggal) {
			$data_order = $this->get_order(array("tanggal"=>$tanggal->tanggal));
			$total = 0;
	...
	function grafik_order_per_bulan()
	{
		...
		$value = [];
		foreach ($res_tanggal as $tanggal) {
			$data_order = $this->get_order(array("perbulan"=>$tanggal->bulantahun));
			$total = 0;
			foreach ($data_order as $order ) {
				$total += ($order->hs * 1) + ($order->gc * 1);
			}
	...
	{
		$this->db->select("*,lo.media as id_media,lo.alamat as alamat,lo.id_cs,lo.id as id,lo.status as id_status,u.first_name as nama_cs,u.phone as no_hp_cs,m.media as media,CASE WHEN status = 1 THEN 'Menunggu Admin' WHEN status = 2 THEN 'Diproses' WHEN status = 3 THEN 'Dikirim' WHEN status = 4 THEN 'Selesai' WHEN status = 5 THEN 'Menunggu Gudang' WHEN status = 6 THEN 'Dibatalkan' END as status,b.bank as nama_bank");
		$this->db->from('laporan_order lo');
		...
		$this->db->where('left(tanggal,7)', $params['perbulan']);	
		}
		...
		$this->db->where('left(tanggal,10) BETWEEN "'.$params['dari'].'" AND "'.$params['sampai'].'"');
		}
	...
	function get_diterima($dari = '', $sampai = '')
	{
		...
		$this->db->where('lo.status = 3 or m.id = 4');		
		$res = $this->db->get()->result();
	...
	function verifikasi_pembayaran($id_order)
	{
		...
		foreach ($res_laporan_order_detail as $detail) {
			$data_order_detail = array(
				...
				"harga_jual"=>$detail->harga_1,				
				"sub_total"=>$detail->harga_1 * $detail->jumlah,
			);
