...
class Member_retail_model extends CI_Model
{
    ...
    function json_kasir($id_toko) {
        ...
        $this->datatables->from('member m');
        $this->datatables->join('users u2','m.id_users=u2.id AND m.id_toko=u2.id_toko');
        $this->datatables->join('kota k', 'k.id=m.id_kota', 'left');
        $this->datatables->join('users u','m.id_sales=u.id_users AND m.id_toko=u.id_toko AND u.level=2', 'left');
        // $this->datatables->where('m.id_toko', $this->userdata->id_toko);
        // $this->datatables->where('u2.id_cabang', $this->userdata->id_cabang);
        // $this->datatables->where('u2.level', 6);
    ...
    function json_sales($id_toko) {
        ...
        $this->datatables->where('u.level', 6);
        return $this->datatables->generate();
    }
    ...
    function get_by_id_toko($id_toko)
    {
        ...
        $this->db->join('users u', 'm.id_sales=u.id_users AND u.id_toko=m.id_toko AND u.level=5', 'left');
    ...
    {
        $this->db->where('id_sales', '0');
        $this->db->where('id_toko', $this->userdata->id_toko);
        if (!empty($id_sales)) {
            $this->db->or_where('id_sales', $id_sales);
            $this->db->where('id_toko', $this->userdata->id_toko);
        }
        $this->db->order_by($this->id_toko, $this->order);
        return $this->db->get($this->table)->result();
