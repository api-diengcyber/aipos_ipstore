...
class Pegawai_retail_model extends CI_Model
{
    ...
    function omzet_sales_tgl($id_users, $tgl, $tgl2='')
    {
        ...
        $row_null = $this->db->select('SUM(od.subtotal) AS omzet')
                            ...
                            ->join('member m', 'o.pembeli=m.id_member AND m.id_toko=o.id_toko', $pos_join_member)
                            ->where($where_cl)
                            ->get()->row();
        if ($row_null) {
        ...
        $row_sales = $this->db->select('SUM(od.subtotal) AS omzet')
                              ->from('orders o')
                              ->join('orders_detail od', 'od.id_orders_2=o.id_orders_2 AND od.id_toko=o.id_toko')
                              ->where($where_cb)
                              ->get()->row();
        if ($row_sales) {
