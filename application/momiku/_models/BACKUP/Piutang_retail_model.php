...
class Piutang_retail_model extends CI_Model
{
    ...
    {
        $this->db->where('id_toko', $this->userdata->id_toko);
        $this->db->where($this->id_pembeli, $id_pembeli);
        $this->db->order_by("DATE(CONCAT(SUBSTRING(deadline,7,4),'-',SUBSTRING(deadline,4,2),'-',SUBSTRING(deadline,1,2))) ASC");
        return $this->db->get($this->table)->result();
    }
    ...
    {
        $this->db->select('piutang.*, member.nama AS pembeli');
        $this->db->from($this->table);
        $this->db->join('member', 'piutang.id_pembeli=member.id');
        $this->db->where('piutang.id_toko', $this->userdata->id_toko);
        $this->db->where("DATE(CONCAT(SUBSTRING(piutang.tanggal,7,4),'-',SUBSTRING(piutang.tanggal,4,2),'-',SUBSTRING(piutang.tanggal,1,2))) BETWEEN '".$tgl1."' AND '".$tgl2."'");
    ...
    {
        $this->db->select('SUM(sisa) AS saldo_piutang');
        $this->db->from($this->table);
        $this->db->where($this->id_pembeli, $id_pembeli);
        $this->db->where('id_toko', $this->userdata->id_toko);
        $this->db->group_by($this->id_pembeli);
        return $this->db->get()->row();
    }
    ...
    function get_piutang_penjualan($jatuh_tempo,$id_toko='',$status=0){
        $this->db->select('p.*,m.*');
        $this->db->from('piutang as p');
        $this->db->join('member as m', 'm.id_member = p.id_pembeli AND m.id_toko=p.id_toko', 'left');
        $this->db->where('p.id_toko', $this->userdata->id_toko);
        if($status==1){
            //Masih 
