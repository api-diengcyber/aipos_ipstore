...
class Member_retail_model extends CI_Model
{
    ...
    function json($id_toko, $kode = '') {
        ...
        $this->datatables->join('users u2', 'm.id_users=u2.id_users AND m.id_toko=u2.id_toko', 'left');
        $this->datatables->join('kota k', 'k.id=m.id_kota', 'left');
        $this->datatables->join('users u','m.id_sales=u.id_users AND m.id_toko=u.id_toko', 'left');
        // $this->db->where_in('u.level', [2,9]); // sales dan leader
        $this->datatables->where('m.id_toko', $id_toko);
        if (!empty($kode)) {
            $this->datatables->where('LEFT(m.kode,2)=', $kode);
        }
        // $this->datatables->where('u2.id_cabang', $this->userdata->id_cabang);
        // $this->datatables->where('(u2.level=6 OR u2.level IS NULL)');
        if($this->userdata->level == 1){
        $this->datatables->add_column('action', anchor(site_url('admin/member_retail/read/$1'),'<button type="button" class="btn btn-xs btn-success"><i class="ace-icon fa fa-check bigger-120"></i></button>')."&nbsp;&nbsp;&nbsp;&nbsp;".anchor(site_url('admin/member_retail/update/$1'),'<button type="button" class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button>')."&nbsp;&nbsp;&nbsp;&nbsp;".anchor(site_url('admin/member_retail/delete/$1'),'<button type="button" class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash bigger-120"></i></button>','onclick="javascript: return confirm(\'Are You Sure ?\')"'), 'id_member');
        }else if($this->userdata->level == 5){
        $this->datatables->add_column('action', anchor(site_url('manager/member_retail/read/$1'),'<button type="button" class="btn btn-xs btn-success"><i class="ace-icon fa fa-check bigger-120"></i></button>')."&nbsp;&nbsp;&nbsp;&nbsp;".anchor(site_url('manager/member_retail/update/$1'),'<button type="button" class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button>')."&nbsp;&nbsp;&nbsp;&nbsp;".anchor(site_url('manager/member_retail/delete/$1'),'<button type="button" class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash bigger-120"></i></button>','onclick="javascript: return confirm(\'Are You Sure ?\')"'), 'id_member');
    ...
    function json_sales($id_toko) {
        ...
        $this->datatables->from('member m');
        $this->datatables->join('users u','m.id_users=u.id_users AND m.id_toko=u.id_toko', 'left');
        $this->datatables->where('m.id_toko', $this->userdata->id_toko);
    ...
    function get_by_id($id)
    {
        ...
        $this->db->join('users u', 'm.id_users=u.id_users AND u.id_toko=m.id_toko', 'left');
    ...
    // get search data by id id toko, term 
    function get_search_by_id_toko($id_toko, $q = '', $limit = 5)
    {
        $this->db->select('m.*, u2.first_name, u2.last_name');
        $this->db->from('member m');
        $this->db->join('users u', 'm.id_users=u.id_users', 'left');
        $this->db->join('users u2', 'm.id_sales=u2.id_users ', 'left');
        $this->db->where('m.id_toko', $id_toko);
        $this->db->like('m.nama', $q, 'both');
