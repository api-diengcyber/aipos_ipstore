...
class Piutang_retail_model extends CI_Model
{
    ...
    function json($id_toko) {
        $this->datatables->select('p.id, p.id_piutang, p.id_toko, p.no_faktur, p.id_pembeli, SUM(p.jumlah_hutang) AS jumlah_hutang, pb.bayar AS jumlah_bayar, (SUM(p.jumlah_hutang)-IFNULL(pb.bayar,0)) AS sisa, p.tanggal, p.deadline, m.nama AS pembeli');
        $this->datatables->from($this->table.' p');
        // $this->datatables->join("users u" ,"p.id_users=u.id_users AND p.id_toko=u.id_toko");
        $this->datatables->join('(SELECT id_toko, id_pembeli, SUM(bayar) AS bayar FROM piutang_bayar GROUP BY id_toko, id_pembeli) AS pb', 'p.id_pembeli=pb.id_pembeli AND p.id_toko=pb.id_toko', 'left');
        $this->datatables->join('member m', 'p.id_pembeli=m.id_member AND m.id_toko=p.id_toko');
        ...
        if ($this->userdata->level == 1) {
            $this->datatables->add_column('action', anchor(site_url('admin/piutang2/read/$1/$2'),'<button class="btn btn-xs btn-success">Lihat</button>')."&nbsp;".anchor(site_url('admin/piutang2/bayar/$1/$2'),'<button class="btn btn-xs btn-primary">Bayar</button>')."&nbsp;".anchor(site_url('admin/piutang2/delete/$1/$2'),'<button class="btn btn-xs btn-danger">Hapus</button>','onclick="javasciprt:return confirm(\'Are You Sure ?\')"'), 'id_pembeli,id_piutang');
        
        } else if($this->userdata->level == 3) {
            $this->datatables->add_column('action', anchor(site_url('outlet/piutang_retail/read/$1/$2'),'<button class="btn btn-xs btn-success">Lihat</button>')."&nbsp;".anchor(site_url('outlet/piutang_retail/bayar/$1/$2'),'<button class="btn btn-xs btn-primary">Bayar</button>')."&nbsp;".anchor(site_url('outlet/piutang_retail/delete/$1/$2'),'<button class="btn btn-xs btn-danger">Hapus</button>','onclick="javasciprt:return confirm(\'Are You Sure ?\')"'), 'id_pembeli,id_piutang');
        }
        $this->datatables->group_by('p.id_pembeli');
    ...
    {
        $this->db->select('p.*, m.nama AS nama_member, m.alamat AS alamat_member, m.uid_akun');
        $this->db->from('piutang p');
        $this->db->join("users u" ,"p.id_users=u.id_users AND p.id_toko=u.id_toko");
        $this->db->join('member m', 'p.id_pembeli=m.id_member AND p.id_toko=m.id_toko', 'left');
        $this->db->where('p.id_toko', $this->userdata->id_toko);
    ...
    // get data by id pembeli
    function get_by_id_pembeli($id_pembeli, $group = false)
    {
        $this->db->select('p.*, m.nama AS nama_member, m.alamat AS alamat_member, m.uid_akun, (SUM(p.jumlah_hutang)-IFNULL(pb.bayar,0)) AS sisa');
        $this->db->from('piutang p');
        // $this->db->join("users u" ,"p.id_users=u.id_users AND p.id_toko=u.id_toko");
        $this->db->join('(SELECT id_toko, id_pembeli, SUM(bayar) AS bayar FROM piutang_bayar GROUP BY id_toko, id_pembeli) AS pb', 'p.id_pembeli=pb.id_pembeli AND p.id_toko=pb.id_toko', 'left');
    ...
    function get_by_between_date($id_toko, $tgl1='', $tgl2='')
    {
        ...
        $this->db->from("piutang p");
        $this->db->join("users u" ,"p.id_users=u.id_users AND p.id_toko=u.id_toko");
        $this->db->join('member m', 'p.id_pembeli=m.id_member AND m.id_toko=u.id_toko');
        $this->db->where('p.id_toko', $this->userdata->id_toko);
        $this->db->where('u.id_cabang', $this->userdata->id_cabang);
