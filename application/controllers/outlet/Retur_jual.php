...
class Retur_jual extends AI_Admin {
    ...
    public function cetak($no_retur = '')
    {
        ...
        if ($row) {
            $data = [
                'toko' => $this->db->where('id', $this->userdata->id_toko)->get('toko')->row(),
                'data_retur' => $row,
            ...
            $this->load->view('admin/retur_jual/retur_cetak', $data, FALSE);
        } else {
	...
	public function read($id)
	{
		...
		if ($row_retur) {
            ...
            $this->view('retur_jual/retur_jual_read', $data);
		} else {
            $this->session->set_flashdata('message', 'Record not found');
            redirect(site_url('admin/retur_jual'));
		}
	}
    ...
    public function tabelTemp()
    {
        ...
        $html = '';
        $row_orders = $this->db->select('o.*, rj.id_retur, rj.no_retur')
                               ->from('orders o')
                               ...
                               ->join('retur_jual rj', 'o.no_faktur=rj.no_faktur AND o.id_toko=rj.id_toko', 'left')
                               ->where('o.id_toko', $this->userdata->id_toko)
                            //    ->where('u.id_cabang', $this->userdata->id_cabang)
        ...
        if ($row_orders) {
            ...
            $this->db->select('od.*, pr.nama_produk, sp.satuan, SUM(rjd.jumlah) AS jumlah_retur');
            $this->db->from('orders_detail od');
            $this->db->join('users u', 'od.id_karyawan=u.id_users AND od.id_toko=u.id_toko', 'left');
            $this->db->join('produk_retail pr', 'od.id_produk=pr.id_produk_2 AND od.id_toko=pr.id_toko');
            $this->db->join('satuan_produk sp', 'pr.satuan=sp.id_satuan AND pr.id_toko=sp.id_toko', 'left');
            $this->db->join('(SELECT * FROM retur_jual_detail WHERE id_retur="'.$row_orders->id_retur.'") rjd', 'od.id_produk=rjd.id_produk AND od.id_toko=rjd.id_toko', 'left');
            $this->db->where('od.id_toko', $this->userdata->id_toko);
            // ->where('u.id_cabang', $this->userdata->id_cabang)
            $this->db->where('od.id_orders_2', $id_orders);
            $this->db->group_by('od.id_orders');
            $res_detail = $this->db->get()->result();
            ...
            foreach ($res_detail as $key) {
                ...
                if ($jumlah_tersedia_u_diretur > 0) {
                    $no++;
                    // $harga_satuan_retur = ($key->sub_total/$key->jumlah);
                    $harga_satuan_retur = $key->harga_satuan;
                    $html .= '
                    <tr>
                        <td>'.$no.'</td>
...
/*
	public function create()
	{
		...
		$data = array(
			'action' => site_url().'admin/retur_jual/create_action',
			'no_retur' => '',
            'id_modul' => $this->userdata->id_modul,
            'button' => 'Selesai',
            'id_toko' => $this->userdata->id_toko,
            ...
            if ($row_faktur) {
                ...
                foreach ($jumlah as $id_orders => $value) {
                    ...
                    if ($row_od && $value > 0) {
                        ...
                        $harga_satuan_retur = round($row_od->sub_total/$row_od->jumlah);
                ...
                if ($total > 0) {
                    ...
                    $this->session->set_flashdata('message', 'Create record failed / Jumlah Retur 0');
                }
            } else {
    ...
    public function create_manual($value='')
    {
        $data = array(
            ...
            'id_modul' => $this->userdata->id_modul,
            'nama_modul' => $this->userdata->nama_modul,
            'action' => site_url().'admin/retur_jual/create_action_manual',
            'no_retur' => '',
            'id_modul' => $this->userdata->id_modul,
            'button' => 'Selesai',
            'id_toko' => $this->userdata->id_toko,
        ...
        $this->view('retur_jual/retur_jual_form_manual', $data);
    }
    ...
    private function cek_stok_produk($barcode)
    {
		...
		$this->db->select('pr.*, '.$this->Mutasi_stok_model->select_stok_mutasi('stok', true));
		$this->db->from('(SELECT * FROM produk_retail WHERE barcode="'.$barcode.'") pr');
    ...
    public function insert_temp_manual()
    {
        ...
        $row_produk = $this->db->select('pr.*')
                               ->from('produk_retail pr')
                               ->join('users u', 'pr.id_users=u.id_users AND pr.id_toko=u.id_toko','left')
                               ->where('pr.id_toko', $this->userdata->id_toko)
                               ->where('pr.barcode', $barcode)
                            //    ->where('u.id_cabang', $this->userdata->id_cabang)
                               ->get()->row();
        if ($row_produk) {
            ...
            $data['status'] = 'error -> '.$stok_ada;
            if ($stok_ada > 0) {
                $data['status'] = 'ok -> '.$stok_ada;
        ...
        foreach ($res_temp as $key) {
            ...
            $html .= '
                <tr>
                    ...
                    <td class="text-center">'.($stok_ada-$key->jumlah).'</td>
    ...
    public function reset_temp()
    {
        ...
        $this->db->where('manual', 1);
        $this->db->delete('retur_jual_temp');
        $this->session->set_flashdata('message', 'Reset data success');
        redirect(site_url('admin/retur_jual/create_manual'));
    }
